...
ForwardIterator max_element(ForwardItertor first, ForwardIterator last, Compare comp)
{
	if(first == last) return first;
	ForwardIterator result = first;
	while(++first != last)
...
template <class ForwardIterator>
ForwardIterator min_element(ForwrardIterator first, ForwardIterator last)
{
	if(first == last) return first;
	ForwardIterator result = first;
	while(++first != last)
...
OutputIterator remove_copy(InputIterator first, InputIteartor last,
...
void __rotate(ForwardIterator first, ForwardIteraror middle, 
...
template<class ForwardIterator1, class ForwardIterator2>
inline ForwardIterator1 search(ForwardIterator first1,
...
pair<ForwardIterator, ForwardIterator>
	__equal_range(ForwardIteraror first, ForwardIterator last, const T& value,
	Distance*, forward_iterator_tag)
{
	Distance len = 0;
	distance(first, last, len);
